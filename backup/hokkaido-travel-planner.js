...
async function searchBlogPostsForTopic(page, topic) {
  ...
  await page.goto('https://search.naver.com/search.naver', { timeout: 60000 });
  ...
  await page.waitForSelector('.total_wrap', { timeout: 60000 });
  ...
  const blogLinks = await page.evaluate(() => {
    ...
    document.querySelectorAll('.total_area').forEach((item) => {
  ...
  for (let i = 0; i < Math.min(blogLinks.links.length, 8); i++) {
    ...
    try {
      ...
      const isRelevant = await page.evaluate((topicQuery) => {
        ...
        const queryTerms = topicQuery.toLowerCase().split(' ');
        // Content is considered relevant if it contains at least 2 of the query terms
        return queryTerms.filter(term => bodyText.includes(term)).length >= 2;
      }, topic.query);
      
      if (!isRelevant) {
        console.log('  - Skipping: Content not relevant to the topic');
        continue;
      }
